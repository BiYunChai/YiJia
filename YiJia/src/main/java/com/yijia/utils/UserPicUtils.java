...
return localPath;
    ...
    * */
    ...
    public static Bitmap toRoundBitmap(Bitmap bitmap, Uri tempUri){
        ...
        //以下有两种方法画圆drawRounRect和drawCircle
        //canvas.drawRounRect(rectF,roundPx,roundPx,paint);//画圆角矩形，第一个参数为图形显示区域，二三个参数分别是水平圆角半径和垂直圆角半径
        canvas.drawCircle(roundPx,roundPx,roundPx,paint);
        paint.setXfermode(new PorterDuffXfermode(PorterDuff.Mode.SRC_IN));//设置两张图片相交时的模式
        canvas.drawBitmap(bitmap,src,dst,paint);//以Mode.SRC_IN模式合并bitmap和已经画了的圆
        return output;
    }
